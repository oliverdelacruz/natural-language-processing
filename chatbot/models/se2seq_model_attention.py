...
class Seq2seq(BasicModel):
    ...
    def _build_graph(self):
        ...
        with tf.device('/gpu:0'):
            ...
            self.labels =  tf.reshape(tf.pad(self.decoder_input[:,1:self.max_sequence], [[0, 0,], [0, 1]], "CONSTANT"), [-1])
            ...
            with tf.name_scope("rnn_layer"):
                ...
                logits = tf.nn.xw_plus_b(tf.reshape(self.decoder_outputs_train.rnn_output,
                                                    [-1, self.rnn_size], name="reshape_logits"), W, b,
                                         name="mul_logits")
    ...
    def _evaluate(self):
        ...
        for idx in range(self.encoder_data_test.shape[0]):
            feed_dict = {self.encoder_input: self.encoder_data_test[idx],
                         ...
                         self.keep_prob: 1.0}
            step, summaries, loss_batch, accuracy_batch = self.session.run(
                [self.global_step, self.dev_summary_op, self.loss, self.accuracy], feed_dict)
            self.dev_summary_writer.add_summary(summaries, step)
            time_str = datetime.datetime.now().isoformat()
            print("{}: step {}, loss {:g}, acc {:g}, perplexity {:g} ".format(
                time_str, step, loss_batch, accuracy_batch, np.exp(loss_batch)))
        ...
        if loss_batch > max(self.average_total_loss[-2:]) and len(self.average_total_loss) > 2:
            tf.assign(self.learning_rate, self.learning_rate.eval() * self.learning_rate_decay_factor)
        ...
        with open(os.path.join(self.out_dir,'validation_measures.csv'), 'a') as f:
            validation_measures = pd.DataFrame([loss_batch, accuracy_batch, np.exp(loss_batch)])
