...
class  Seq2seq_att_luong(BasicModel):
    # Build the graph
    def _build_graph(self):
        ...
        with tf.device('/gpu:0'):
            ...
            self.encoder_input = tf.placeholder(tf.int32, [self.batch_size, None],
                                                name="input_encoder")
            self.decoder_input = tf.placeholder(tf.int32, [self.batch_size, None],
                                                name="input_decoder")

            # The length of the sentences
            self.encoder_length = tf.placeholder(tf.int32, [self.batch_size], name="sentence_length_encoder")
            self.decoder_length = tf.placeholder(tf.int32, [self.batch_size], name="sentence_length_decoder")
            ...
            with tf.name_scope("rnn-encoder"):
                ...
                gru_bw = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.rnn_size),
                                                       output_keep_prob=self.keep_prob)
                encoder_outputs, encoder_state = tf.nn.bidirectional_dynamic_rnn(gru_fw, gru_bw, emb_words_encoder,
                                                                                 sequence_length=self.encoder_length,
                                                                                 dtype=tf.float32)

                # print(tf.reshape(tf.concat(encoder_state,1), [-1,1,self.rnn_size]))
                gru_encoder_context = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.rnn_size * 2),
                                                            output_keep_prob=self.keep_prob, dtype=tf.float32)
                # The initial state
                batch_size = tf.shape(self.encoder_input)[0]
                gru_encoder_context_zero = gru_encoder_context.zero_state(batch_size=batch_size, dtype=tf.float32)
                #gru_encoder_context_zero.clone(cell_state= tf.zeros([None ,self.rnn_size * 2]))
                #print(gru_encoder_context_zero.get_shape())
                #initial_state = tf.Variable(tf.zeros([None ,self.rnn_size * 2]), trainable=False, validate_shape=False, name="context_state")
                #print(initial_state.get_shape())
                print(gru_encoder_context_zero.get_shape())
                # The initial state
                initial_state = tf.Variable(tf.zeros([self.batch_size, gru_encoder_context.state_size]), trainable=False,
                                            name="state")
                # The input words
                # print(encoder_state.get_shape())
                #print(tf.reshape(tf.concat(encoder_state,1),[-1,1,self.rnn_size]).get_shape)
                # print(tf.reshape(tf.concat(encoder_state, 1), [-1, 1, self.rnn_size]).get_shape())
                encoder_outputs_context, encoder_state_context = tf.nn.dynamic_rnn(gru_encoder_context, tf.reshape(tf.concat(encoder_state,1), [-1,1,self.rnn_size * 2]),
                                                                                   initial_state=initial_state)
                # Save the initial state
                with tf.control_dependencies([tf.assign(initial_state, encoder_state_context)]):
                    lstm_decoder = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.rnn_size * 2),
                                                                 output_keep_prob=self.keep_prob)
                    ...
                    with tf.name_scope("rnn_decoder"):
                        ...
                        with tf.variable_scope("decoder_helper") as scope:
                            ...
                            attention_mechanism = tf.contrib.seq2seq.LuongAttention(
                                self.rnn_size * 2, tf.concat(encoder_outputs, 2),
                                name='LuongAttention')
                            ...
                            init_state = attn_zero.clone(cell_state=encoder_state_context)
            ...
            with tf.name_scope("rnn_layer"):
                ...
                W = tf.get_variable("W", [self.rnn_size*2, self.vocab_size], tf.float32,
                ...
                logits = tf.nn.xw_plus_b(tf.reshape(self.decoder_outputs_train.rnn_output,
                                                    [-1, self.rnn_size * 2], name="reshape_logits"), W, b, name ="mul_logits")
    ...
    def _evaluate(self):
        ...
        self.average_total_loss.append(np.mean(np.array(self.cum_test_loss)))
        ...
        with open(os.path.join(self.out_dir,'validation_measures.csv'), 'a') as f:
            validation_measures = pd.DataFrame([loss_batch, np.exp(loss_batch)])
