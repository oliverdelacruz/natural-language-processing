        ...
        with tf.device('/gpu:0'):
            ...
            with tf.name_scope("rnn_decoder"):
                ...
                with tf.variable_scope("decoder_helper") as scope:
                    ...
                    output_layer = layers_core.Dense(self.rnn_size, use_bias=True,
                    ...
                    decoder_infer = tf.contrib.seq2seq.BasicDecoder(cell=lstm_decoder, helper=helper_infer,
                                                                initial_state=encoder_state,
                                                                output_layer=output_layer)
                    decoder_outputs_infer, decoder_state_infer, final_sequence_lengths_infer = \
                        tf.contrib.seq2seq.dynamic_decode(decoder=decoder_infer,
                                                          output_time_major=False,
                                                          impute_finished=True,
                    ...
                    decoder_beam = tf.contrib.seq2seq.BeamSearchDecoder(cell=lstm_decoder,
                                                                        embedding=emb, start_tokens= tf.tile([1], [64]),
                                                                        end_token=2,
                                                                        initial_state=encoder_state,
                                                                        beam_width=200,
                                                                        output_layer=output_layer)
                    decoder_outputs_beam, decoder_state_beam, final_sequence_lengths_beam = tf.contrib.seq2seq.dynamic_decode(
                             ...
                             output_time_major=False,
                             impute_finished=True)
                    # self.decoder_outputs_beam = tf.reshape(decoder_outputs_beam.rnn_output,
                    #                                        [-1, self.rnn_size],
            ...
            with tf.name_scope("down_project"):
                W_down = tf.get_variable("W_down", [self.rnn_size_reduced, self.rnn_size], tf.float32,
                                         initializer=tf.contrib.layers.xavier_initializer())
                outputs = tf.matmul(tf.reshape(self.decoder_outputs_train.rnn_output, [-1, self.rnn_size])
                                    ,tf.transpose(W_down), name="mul_project")
                print('Printing output tensor size: {} - reduced layer'.format(str(outputs.get_shape())))
            ...
            with tf.name_scope("rnn_layer"):
                ...
                logits = tf.nn.xw_plus_b(outputs, W, b, name ="mul_logits")
