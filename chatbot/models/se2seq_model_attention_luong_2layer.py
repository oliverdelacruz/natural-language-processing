    ...
    def _build_graph(self):
        ...
        with tf.device('/gpu:0'):
            ...
            with tf.name_scope("rnn-encoder"):
                lstm_encoder = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.GRUCell(self.rnn_size) for _ in range(2)]),
            ...
            with tf.name_scope("rnn_decoder"):
                lstm_decoder = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.GRUCell(self.rnn_size) for _ in range(2)]),
                                                             output_keep_prob = self.keep_prob)
                with tf.variable_scope("decoder_helper") as scope:
                    ...
                    decoder_infer = tf.contrib.seq2seq.BasicDecoder(cell=attn_cell, helper=helper_infer,
                                                                    initial_state=init_state)
                    ...
                    self.decoder_outputs_infer = tf.reshape(decoder_outputs_infer.rnn_output, [-1, self.rnn_size],
                                                            name="reshape_logits_infer")
                print('Printing decoder outputs tensor size: {}'.format(str(self.decoder_outputs_train.rnn_output.get_shape())))

            # Down-projection of the h
            with tf.name_scope("down_project"):
                W_down = tf.get_variable("W_down", [self.rnn_size,self.rnn_size_reduced], tf.float32,
                                         initializer=tf.contrib.layers.xavier_initializer())
                outputs = tf.matmul(tf.reshape(self.decoder_outputs_train.rnn_output, [-1, self.rnn_size]),W_down, name ="mult_down")

            # Fully connected layer
            with tf.name_scope("rnn_layer"):
                # Parameters to calibrate
                W = tf.get_variable("W", [self.rnn_size_reduced, self.vocab_size], tf.float32,
                                    initializer=tf.contrib.layers.xavier_initializer())
                b = tf.get_variable("b", [self.vocab_size], tf.float32, initializer=tf.zeros_initializer())

                # Calculate logits and predictions
                logits = tf.nn.xw_plus_b(tf.reshape(outputs, [-1, self.rnn_size_reduced], name="reshape_logits"), W, b,
                                         name="mul_logits")
    ...
    def train(self):
        ...
        for n_epoch in range(self.num_epoch):
            ...
            for idx in range(len(encoder_inputs)):
                ...
                if step % 2000 == 0:
                    if len(cum_loss) > 2:
                        if np.mean(np.array(avg_loss).flatten()) > max(cum_loss[-2:]):
                            self.session.run(self.learning_rate_decay_op)
                            print("Changing the learning rate")
                        cum_loss.append(np.mean(np.array(avg_loss).flatten()))
                        avg_loss = []
                if step % 5000 == 0 and step > 45000:
                    self.session.run(self.learning_rate_decay_op)
                    print("Changing the learning rate")
